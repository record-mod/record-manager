...
class PathManager(
    ...
    /**
     * The ReCord folder where shared ReCord files are stored.
     * Standard path: `~/ReCord`
     */
    val recordDir = Environment.getExternalStorageDirectory().resolve("ReCord")

    /**
     * Global keystore used for signing APKs.
     */
    val keystoreFile = recordDir.resolve("ks.keystore")
    ...
    fun cachedReCordXposed(version: SemVer) = patchingDownloadDir
        .resolve("record-xposed")
        .resolve("$version.apk")
