package dev.tralwdwd.record.manager.installer.session
...
import dev.tralwdwd.record.manager.ui.activity.MainActivity
...
class InstallService : Service() {
    ...
    override fun onStartCommand(intent: Intent, flags: Int, startId: Int): Int {
        val isInstall = intent.action == "record.actions.ACTION_INSTALL"
        when (val statusCode = intent.getIntExtra(PackageInstaller.EXTRA_STATUS, -999)) {
            ...
            PackageInstaller.STATUS_FAILURE_ABORTED -> if (isInstall) showToast(R.string.installer_aborted)
                ...
                if (isInstall) {
                    ...
                    startActivity(
                        Intent("record.actions.ACTION_INSTALL_FINISHED").apply {
                            setClass(this@InstallService, MainActivity::class.java)
                            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                            putExtra("record.extras.EXTRA_MESSAGE", intent.getStringExtra(PackageInstaller.EXTRA_STATUS_MESSAGE))
                        }
                    )
                }
            }
